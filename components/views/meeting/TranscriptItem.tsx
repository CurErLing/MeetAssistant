...
interface TranscriptItemProps {
  ...
  inputValue?: string;
  readOnly?: boolean;
  searchQuery?: string; // Add highlight search query
...
export const TranscriptItem: React.FC<TranscriptItemProps> = React.memo(({
  segment, speaker, isActive, isEditing = false, inputValue = "", readOnly = false, searchQuery = "",
      ...
      <>
        {parts.map((part, i) => 
          part.toLowerCase() === searchQuery.toLowerCase() ? (
            <mark key={i} className="bg-yellow-200 text-slate-900 rounded-sm px-0.5 mx-0.5 font-bold">
              {part}
            </mark>
          ) : (
            part
  ...
  return (
    <div 
      ...
      // Add data attribute for search scroll targeting
      data-has-match={searchQuery && segment.text.toLowerCase().includes(searchQuery.toLowerCase()) ? "true" : undefined}
