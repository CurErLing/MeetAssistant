...
export const TranscriptItem: React.FC<TranscriptItemProps> = React.memo(({
  ...
  >
    ...
    <div className="flex-shrink-0 w-full sm:w-24 flex sm:block items-center justify-between sm:justify-start">
       {readOnly || !onSpeakerClick ? (
          <div className={`text-xs font-bold sm:mb-1 block truncate ${speaker.color ? speaker.color.split(' ')[0] : 'text-slate-700'}`}>
            {speaker?.name || "未知"}
          </div>
       ) : (
          <button 
            onClick={(e) => { e.stopPropagation(); onSpeakerClick(speaker.id); }}
            className={`text-xs font-bold sm:mb-1 block hover:underline text-left truncate ${speaker.color ? speaker.color.split(' ')[0] : 'text-slate-700'}`}
          >
            {speaker?.name || "未知"}
          </button>
       )}
